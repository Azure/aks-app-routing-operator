...
func TestLoadCRDs(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "default domain enabled", cfg: defaultDomainEnabled, expectedCRDNames: slices.Concat(nginxCrds, []string{clusterExternalDnsCrdName}, defaultDomainCertificates)},
...
func TestShouldLoadCRD(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "external dns crd with default domain enabled", cfg: defaultDomainEnabled, filename: externalDnsCrdFilename, expected: false},
